...
namespace farmtrax {
    ...
    Divy::Divy(std::shared_ptr<Field> field, DivisionType type, std::size_t machines)
    ...
    Divy::Divy(std::shared_ptr<Part> part, DivisionType type, std::size_t machines)
    ...
    void Divy::set_machine_count(std::size_t machines) {
        if (machines == 0)
            throw std::invalid_argument("machine count > 0");
        machine_count_ = machines;
    }

    void Divy::set_division_type(DivisionType type) { division_type_ = type; }

    const DivisionResult &Divy::result() const { return division_; }

    void Divy::compute_division() {
