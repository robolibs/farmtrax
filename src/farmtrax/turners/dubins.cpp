...
namespace farmtrax {
    namespace turners {
        ...
        DubinsPath::DubinsPath() : total_length(0.0) {}

        // DubinsStateSpace::DubinsPath implementation
        double DubinsStateSpace::DubinsPath::length() const { return length_[0] + length_[1] + length_[2]; }

        // DubinsStateSpace implementation
        DubinsStateSpace::DubinsStateSpace(double turningRadius) : rho_(turningRadius) {}

        double DubinsStateSpace::distance(double q0[3], double q1[3]) { return rho_ * dubins(q0, q1).length(); }

        DubinsStateSpace::DubinsPath DubinsStateSpace::dubins(double q0[3], double q1[3]) const {
        ...
        void DubinsStateSpace::interpolate(double q0[3], DubinsPath &path, double seg, double s[3]) const {
            if (seg < 0.0)
                seg = 0.0;
            if (seg > path.length())
                seg = path.length();
        ...
        void DubinsStateSpace::sample(double q0[3], double q1[3], double step_size, double &length,
        ...
        // Dubins class implementation
        Dubins::Dubins(double min_turning_radius) : radius_(min_turning_radius), ds_(min_turning_radius) {}

        DubinsPath Dubins::plan_path(const concord::Pose &start, const concord::Pose &end, double step_size) const {
        ...
        std::vector<DubinsPath> Dubins::get_all_paths(const concord::Pose &start, const concord::Pose &end,
        ...
        double Dubins::mod2pi(double x) const {
            if (x < 0 && x > DUBINS_ZERO)
                return 0;
            double xm = x - twopi * floor(x / twopi);
            if (twopi - xm < .5 * DUBINS_EPS)
        ...
        DubinsStateSpace::DubinsPath Dubins::dubinsLSL(double d, double alpha, double beta) const {
            double ca = cos(alpha), sa = sin(alpha), cb = cos(beta), sb = sin(beta);
            double tmp = 2. + d * d - 2. * (ca * cb + sa * sb - d * (sa - sb));
            if (tmp >= DUBINS_ZERO) {
                double theta = atan2(cb - ca, d + sa - sb);
        ...
        DubinsStateSpace::DubinsPath Dubins::dubinsRSR(double d, double alpha, double beta) const {
            double ca = cos(alpha), sa = sin(alpha), cb = cos(beta), sb = sin(beta);
            double tmp = 2. + d * d - 2. * (ca * cb + sa * sb - d * (sb - sa));
            if (tmp >= DUBINS_ZERO) {
                double theta = atan2(ca - cb, d - sa + sb);
        ...
        DubinsStateSpace::DubinsPath Dubins::dubinsRSL(double d, double alpha, double beta) const {
            double ca = cos(alpha), sa = sin(alpha), cb = cos(beta), sb = sin(beta);
            double tmp = d * d - 2. + 2. * (ca * cb + sa * sb - d * (sa + sb));
            if (tmp >= DUBINS_ZERO) {
                double p = sqrt(std::max(tmp, 0.));
        ...
        DubinsStateSpace::DubinsPath Dubins::dubinsLSR(double d, double alpha, double beta) const {
            double ca = cos(alpha), sa = sin(alpha), cb = cos(beta), sb = sin(beta);
            double tmp = -2. + d * d + 2. * (ca * cb + sa * sb + d * (sa + sb));
            if (tmp >= DUBINS_ZERO) {
                double p = sqrt(std::max(tmp, 0.));
        ...
        DubinsStateSpace::DubinsPath Dubins::dubinsRLR(double d, double alpha, double beta) const {
            double ca = cos(alpha), sa = sin(alpha), cb = cos(beta), sb = sin(beta);
            double tmp = .125 * (6. - d * d + 2. * (ca * cb + sa * sb + d * (sa - sb)));
            if (fabs(tmp) < 1.) {
                double p = twopi - acos(tmp);
        ...
        DubinsStateSpace::DubinsPath Dubins::dubinsLRL(double d, double alpha, double beta) const {
            double ca = cos(alpha), sa = sin(alpha), cb = cos(beta), sb = sin(beta);
            double tmp = .125 * (6. - d * d + 2. * (ca * cb + sa * sb - d * (sa - sb)));
            if (fabs(tmp) < 1.) {
                double p = twopi - acos(tmp);
